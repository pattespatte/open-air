...
/* --- View-specific styles ---
   Injected once per session. The shared breathing & grounding primitives
   (.breath-circle, .breath-*, .grounding-*, .reassurance-body) live in
   css/style.css because Practice reuses them too — keeping them here broke
   Practice when NOW was never visited. Only NOW-only classes stay below. */
const NOW_STYLES = `
.now-section {
...
/* The breathing circle is shared; the CTA button wrapper is NOW-only. */
.breathing-cta {
  background: none;
  border: none;
  cursor: pointer;
